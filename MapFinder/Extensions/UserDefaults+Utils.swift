...
extension UserDefaults {
    struct UserDefaultsFavorites {
        static let kFavorites = "Favorites"
    }
    var placesFavorties: [Place] {
        get {
            guard let places = get(objectType: [Place].self, forKey: UserDefaultsFavorites.kFavorites) else { return [] }
            return places
        }
        set {
            set(object: newValue, forKey: UserDefaultsFavorites.kFavorites)
            synchronize()
        }
    }
